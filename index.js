...
app.post("/ussd", (req, res) => {
  const { text } = req.body;
  let inputs = text.split("*").filter((i) => i !== ""); // remove empty strings
  const level = inputs.length;
  let response = "";

  // If session is just starting
  if (text === "") {
...
2. Kinyarwanda`;
    ...
    if (inputs.includes("0")) {
      // Remove last input and recalculate
      const backIndex = inputs.lastIndexOf("0");
      inputs = inputs.slice(0, backIndex);
      const newText = inputs.join("*");
      req.body.text = newText;
      res.redirect(307, "/ussd"); // 307 preserves POST
      return;
    }
